...
void drawFullScreenImageDMA(const u16 *image) {
  volatile u16 *picture = (volatile u16 *) image;
  DMA[3].src = &picture[OFFSET(0, 0, WIDTH)];
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  volatile u16 *picture = (volatile u16 *) image;
  for (int r = 0; r < height; r++) { //for loop encompassing all possible values for height
    DMA[3].src = &picture[OFFSET(r, 0, width)];
    DMA[3].dst = &videoBuffer[OFFSET(row+r,col,WIDTH)];
